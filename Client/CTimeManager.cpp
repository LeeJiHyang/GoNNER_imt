...
CTimeManager::CTimeManager()
{
	...
	/*컴퓨터의 메인보드에는 하나의 고해상도 타이머가 존재합니다.
	이 타이머는 정확하고 일정한 주파수(즉, 1초에 각 타이머의 성능에 맞는 진동수)를 갖기 때문에
	측정하고자 하는 코드들의 시작과 끝에서 CPU 클럭수를 얻어 그 차로 수행시간을 얻을 수 있습니다.*/
...
{
	//(_T("FPS: %d"), _frameRate);
	//(_T("World: %f"), _worldTime);
	//(_T("SecPerFrameTime: %f"), _timeElapsed);
}
